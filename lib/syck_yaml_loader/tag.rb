...
class Module
    ...
    def syck_yaml_as( tag, sc = true )
        ...
        class_eval <<-"END", __FILE__, __LINE__+1
            ...
            def taguri
                if respond_to? :to_yaml_type
                    SyckYamlLoader.tagurize( to_yaml_type[1..-1] )
                else
                    return @taguri if defined?(@taguri) and @taguri
                    tag = #{ tag.dump }
                    if self.class.yaml_tag_subclasses? and self.class != SyckYamlLoader.tagged_classes[tag]
        ...
        END
        SyckYamlLoader.tag_class tag, self
    ensure
        $VERBOSE = verbose
    end
