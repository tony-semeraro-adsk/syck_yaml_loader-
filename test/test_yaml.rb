...
require 'helper'
require 'date'
require 'syck_yaml_loader/ypath'
...
class YAML_Unit_Tests < Test::Unit::TestCase
	...
	# 	assert_equal( obj, SyckYamlLoader::load( yaml ), msg )
	# 	assert_equal( obj, SyckYamlLoader::parse( yaml ).transform, msg )
  #       assert_equal( obj, SyckYamlLoader::load( obj.to_yaml ), msg )
	# 	assert_equal( obj, SyckYamlLoader::parse( obj.to_yaml ).transform, msg )
  #       assert_equal( obj, SyckYamlLoader::load(
    ...
    def assert_path_segments( path, segments, msg = nil )
        SyckYamlLoader::YPath.each_path( path ) { |choice|
...
EOY
	...
	def test_spec_log_file
		doc_ct = 0
		SyckYamlLoader::load_documents( <<EOY
---
...
EOY
	...
	def test_spec_domain_prefix
        ...
        SyckYamlLoader.add_domain_type( "domain.tld,2002", 'invoice', &customer_proc )
        SyckYamlLoader.add_domain_type( "domain.tld,2002", 'customer', &customer_proc )
...
# 		doc_ct = 0
# 		SyckYamlLoader::parse_documents( <<EOY
...
# 	end
	...
	def test_spec_url_escaping
		SyckYamlLoader.add_domain_type( "domain.tld,2002", "type0" ) { |type, val|
			"ONE: #{val}"
		}
		SyckYamlLoader.add_domain_type( "domain.tld,2002", "type%30" ) { |type, val|
			"TWO: #{val}"
		}
		assert_parse_only(
...
EOY
	...
	def test_spec_explicit_families
        SyckYamlLoader.add_domain_type( "somewhere.com,2002", 'type' ) { |type, val|
            "SOMEWHERE: #{val}"
        }
		assert_parse_only(
...
EOY
	...
	def test_spec_application_family
		...
		SyckYamlLoader.add_domain_type( "clarkevans.com,2002", 'graph/shape' ) { |type, val|
			if Array === val
				val << "Shape Container"
				val
			else
		...
		SyckYamlLoader.add_domain_type( "clarkevans.com,2002", 'graph/circle', &one_shape_proc )
		SyckYamlLoader.add_domain_type( "clarkevans.com,2002", 'graph/line', &one_shape_proc )
		SyckYamlLoader.add_domain_type( "clarkevans.com,2002", 'graph/text', &one_shape_proc )
		assert_parse_only(
...
# 	end
	...
	#
	def test_document
		y = SyckYamlLoader::Stream.new( :Indent => 2, :UseVersion => 0 )
		y.add(
			{ 'hi' => 'hello', 'map' =>
				{ 'good' => 'two' },
			  'time' => Time.now,
...
#     end
    ...
    def test_circular_references
        ...
        inspect_str = "[[...], [...]]"
				#a.to_yaml #  => "--- &id001 \n- *id001\n- *id001\n"
        assert_equal( inspect_str, SyckYamlLoader::load( "--- &id001 \n- *id001\n- *id001\n" ).inspect )
    end
    ...
    #   omap = SyckYamlLoader::Omap.new
    #   1000.times { |i| omap["key_#{i}"] = { "value" => i } }
    #   raise "id collision in ordered map" if omap.to_yaml =~ /id\d+/
    # end

    def test_date_out_of_range
      assert_nothing_raised{SyckYamlLoader::load('1900-01-01T00:00:00+00:00')}
    end

    def test_normal_exit
			SyckYamlLoader.load("2000-01-01 00:00:00.#{"0"*1000} +00:00\n")
      # '[ruby-core:13735]'
    end
end
end
